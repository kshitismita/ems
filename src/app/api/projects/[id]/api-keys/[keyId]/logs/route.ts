...
export async function GET(
    ...
    try {
        ...
        const logs = await ApiKeyLog.find({ apiKeyId: new mongoose.Types.ObjectId(keyId) })
            .sort({ timestamp: -1 })
            .skip(skip)
            .limit(limit);

        const total = await ApiKeyLog.countDocuments({ apiKeyId: new mongoose.Types.ObjectId(keyId) });
